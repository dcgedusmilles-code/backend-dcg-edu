    ...
    try {
      ...
      if (!email || !nif || !mobile || !name) {
        return res.status(400).json({
          message: "Os campos Name, email, NIF e número de telefone são obrigatórios.",
        });
      }
      ...
      if (alreadyExist) {
        ...
        else if (alreadyExist.mobile === mobile) duplicateField = 'número de telefone';  
        return res.status(400).json({ message: `O ${duplicateField} já está em uso.` });
      }
