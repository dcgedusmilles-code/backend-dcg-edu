...
const createService = asyncHandler(async (req, res) => {
  ...
  if (!title || !description || !category) {
    return res
      .status(402)
      .json({ message: "title, description and category is required" });
  }
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: "Error to create service,=>" + error?.message });
  }
});
...
const getAllService = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ error: "Error to view services list =>" + error?.message });
  }
});
...
const getOneService = asyncHandler(async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ message: "bad request, please enter winth id service" });
  }
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: "erro ao pegar servico " + error?.message });
  }
});
...
const updateService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: "Error updating service: " + error.message });
  }
});
...
const deleteService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!removedService) {
      return res
        .status(404)
        .json({ message: "Service could not be deleted, not found" });
    }
    ...
    return res
      .status(400)
      .json({ error: "Error deleting service: " + error.message });
  }
});
