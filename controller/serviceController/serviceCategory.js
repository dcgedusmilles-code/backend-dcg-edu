...
const getOneServiceCategory = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: "Erro ao pegar categoria," + error.message });
  }
});
...
const updateCategoryService = asyncHandler(async (req, res) => {
  ...
  try {
    const updateCategoryService = await Category.findByIdAndUpdate(
      id,
      req.body,
      {
        new: true,
    ...
    return res
      .status(400)
      .json({ error: "erro ao actualizar categoria " + error?.message });
  }
});
...
const deleteCategoryService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!removedCategory) {
      return res
        .status(404)
        .json({ message: "Category could not be deleted, not found" });
    }
